...
class CapitalizationService extends BaseService {
  run = (data: IAmortizationRequest) => {
    return new Promise<IAmortizationResponse>(async (resolve, reject) => {
      const { capital, tax, installments, installmentsValue, amount } = data;
      const body: IBody = {};
      ...
      try {
        const response = await this.axios.post('/tools/capitalization', body);
