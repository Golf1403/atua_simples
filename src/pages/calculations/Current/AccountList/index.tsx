...
import currentAccountSchema from '@/validators/components/currentAccountSchema';
import { currentAccount as onShortcuts } from '@/utils/shortcuts';
import useCurrentAccount, { pagination } from '@/hooks/currentAccount';
...
const CustomForm = (): JSX.Element => {
  const { listAccountsByTypeId } = useCurrentAccount();
  ...
  return (
    <Fragment>
      <GenerateToolbar />
      <GenerateSidebar title={labelsEnum.CURRENT_ACCOUNT} />
...
const CurrentList = (): JSX.Element => {
  return (
    <Formik validationSchema={currentAccountSchema} initialValues={initialValues} onSubmit={() => {}}>
      <CustomForm />
    </Formik>
  );
};
