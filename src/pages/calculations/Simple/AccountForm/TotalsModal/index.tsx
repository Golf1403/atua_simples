...
const TotalsModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  useEffect(() => {
    const total = summary.reduce((acc: any, sum: any) => (sum.year || 0) + acc, 0);
    const totalAround = summary.reduce((acc: any, sum: any) => Math.ceil(sum.year || 0) + acc, 0);
  ...
  return (
    <DefaultModal isOpen={isOpen} onCopy={copyTableToClipboard} onCancel={onClose} onClose={onClose} title="Resumo">
      <Table ref={tableRef}>
        <TableBody>
          {summary.map((description: any, index: any) => (
            <Row key={index}>
