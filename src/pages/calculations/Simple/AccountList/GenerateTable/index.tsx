...
import { editSimpleUpdatePage, newSimpleUpdatePage } from '@/Routes/pages/calculations';
...
const GenerateTable = (): JSX.Element => {
  ...
  const handleReloadConfirmModal = async (isReload?: boolean) => {
    try {
      if (values.newAccount) {
        newAccount();
        history.push(newSimpleUpdatePage.path, { force: true, isCharged: true, reset: true });
        return;
      }
  ...
  const handleEditAccount = useCallback(
    (accountId: string) => {
      const editPath = editSimpleUpdatePage.path.split(':')[0];
  ...
  const handleDeleteAccount = useCallback(
    async (accountId: string) => {
      try {
        ...
        const account = list.find((account: any) => account.id == accountId);
        if (!account) return;
  ...
  const generateTable = useCallback(() => {
    ...
    const dataTable = list.map((accountOfList: any) => {
  ...
  const confirmModal = useCallback(async () => {
    try {
      if (list && isOpen) {
        ...
        const newAccounts = list.filter((item: any) => item.id !== deleted.id);
        setFieldValue('data', newAccounts);
        setAccount((account: any) => ({ ...account, list: newAccounts }));
        await reload();
      }
    } catch (error) {
