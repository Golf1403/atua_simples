...
import useSimpleUpdate, { initialFeeFines } from '@/hooks/simpleUpdate';
...
const GenerateShortCuts = ({ onShortcuts }: { onShortcuts: Function }): JSX.Element => {
  ...
  const addAuthor = () => setLayout((layout: any) => ({ ...layout, modalAddAuthor: { visible: true } }));
  ...
  const deleteAll = () => {
    const currentAuthor = author.list.map((_author: any, index: any) => {
