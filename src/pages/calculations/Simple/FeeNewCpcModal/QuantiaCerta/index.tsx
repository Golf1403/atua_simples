...
import InterestFineConfigModal from '../../InterestForm/ConfigModal';
...
const QuantiaCerta = ({
  ...
  const calculate = async () => {
    ...
    if (!values.indexId || values.indexId === -1) return;
    if (!allMemcalcs[values.indexId]?.length) return;
  ...
  return (
    <Container onBlur={calculate}>
      ...
      <Content>
        ...
        <Interest>
          <DateContainer>
            <DefaultDateInput disabled={disabled} label="Transito julgado" name="dateStart" />
          </DateContainer>
